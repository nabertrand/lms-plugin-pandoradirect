package Plugins::PandoraDirect::API;
...
use constant API_URL   => 'https://api.pandoradirect.com/v1/';
...
sub channelFilters {
	...
	# Check cache
	if ( my $cached = $cache->get('pandoradirect_channel_filters_' . $network) ) {
		return $cb->($cached);
	}
	...
	_call(
		...
		sub {
			if ( my $res = shift ) {
				$cache->set( 'pandoradirect_channel_filters_' . $network, $res, CACHE_TTL );
				$cb->( $res || [] );
			}
			else {
				$cb->(undef);
